...
TouchableOpacity, ScrollView, Alert,AsyncStorage, BackHandler,Platform,Keyboard,Linking
} from "react-native";
...
seyahatPuanla() {
  ...
  componentWillUnmount(){
	...
	getOdemeAsama1 = ()=>{
		 Alert.alert("Bilgilendirme","Seçilen aracın konumunuza yönlendirilmesini istiyor musunuz?",[
              {text: 'İstiyorum', onPress: () => {
                          ...
                          }).then((response) => response.json()).then((jr) =>
                      {
                          if(jr["status"] == 1){
                              this.marqueetext = jr.mesaj;
                              this.setState({secimasama:false,toplamucret: (parseFloat(this.state.aradakimesafe)*parseFloat(this.state.kmbasiucret)).toFixed(1),asama2gec:true,seyehatlistesi:jr.travel_list, neredentext:null, nereyetext:null});
                          }else{
                              Alert.alert("Bilgilendirme",jr.message,[
                                  {text: 'Kapat', onPress: () => null}
                              ]);
    ...
    return (
      ...
      />
        <Header transparent style={styles.header} androidStatusBarColor='#ffffff'>
          ...
          <Right style={{marginTop:-10,alignItems:"center"}}>
		        <TouchableOpacity onPress={()=>{Linking.openURL(`tel:08502411147`)}}>
                <Image source={require('../../../assets/phone.png')} style={{width: 30,marginRight: 10,marginTop: 4}} resizeMode="contain" ></Image>
              </TouchableOpacity>
  ...
  {!this.state.loader?
        ...
        <Item style={this.state.neredentext && this.state.nereyetext? styless.adresarasecim : styless.adresara}>
                   ...
                   onPress={() => this.props.navigation.navigate("haritaSecim", {veriIsle:this.openSearchModal,myKonum:this.myKonum})}
                 >
      ...
      }
{
  ...
  <Item style={styless.nereyearaadim2}>
              ...
              activeOpacity={1}
             style={styles.touchable}
             onPress={() => this.props.navigation.navigate("haritaSecim", {veriIsle:this.openSearchModalNereye,myKonum:this.myKonum1})}
           >
...
{ this.state.neredentext && this.state.nereyetext && !this.state.asama2gec && !this.state.secimasama?
